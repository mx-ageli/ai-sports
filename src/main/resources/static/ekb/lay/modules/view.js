    ...
    .define(
        ...
        function (exports) {
            ...
            self.loading = function (elem) {
                elem.append(
                    (this.elemLoad = $(
                        '<i class="layui-anim layui-anim-rotate layui-anim-loop layui-icon layui-icon-loading ekb-loading"></i>'
                    ))
                )
            };
            ...
            self.modal.base = function (msg, params) {
                ...
                params = $.extend(
                    {
                        skin: 'layui-layer-admin-modal ekb-alert',
            ...
            self.tab = {
                ...
                init: function () {
                    var tab = this;
                    var btnCls = tab.wrap + ' .ekb-tabs-btn';
                    ...
                    layui.dropdown.render({
                        ...
                        click: function (name) {
                            ...
                            if (name === 'current') {
                                tab.del(layui.ekb.route.fileurl);
                            }
                            if (name === 'refresh') {
                                tab.refresh();
                            }
                    ...
                    $(document).on('click', btnCls, function (e) {
                        ...
                        if ($(e.target).hasClass('ekb-tabs-close')) {
                            tab.del(url)
                        } else {
                    ...
                    $('.ekb-tabs-hidden').addClass('layui-show');
                    this.isInit = true
                },
                has: function (url) {
                    var exists = false;
                ...
                delAll: function (type) {
                    var tab = this;
                    var menuBtnClas = tab.menu + ' .ekb-tabs-btn';
                ...
                del: function (url, backgroundDel) {
                    ...
                    var lay = '[lay-url="' + url + '"]';
                    var thisBody = $(
                        '#' + conf.containerBody + ' > .ekb-tabs-item' + lay
                    );
                    ...
                    if (backgroundDel === undefined) {
                        if (thisMenu.hasClass('ekb-tabs-active')) {
                            $(this.menu + ' li:last').click()
                        }
                    }
                },
                refresh: function (url) {
                    url = url || layui.ekb.route.fileurl;
                    if (this.has(url)) {
                        this.del(url, true);
                        self.renderTabs(url)
                    }
                ...
                clear: function () {
                    this.data = [];
                    this.isInit = false;
                    $(document).off('click', this.wrap + ' .ekb-tabs-btn')
                },
                change: function (route, callback) {
                    ...
                    if (existsTab) {
                        ...
                        layui.ekb.navigate(route.href)
                    } else {
                        self.loadHtml(fileurl, function (res) {
                            var htmlElem = $(
                                "<div><div class='ekb-tabs-item' lay-url='" +
                                fileurl +
                                "'>" +
                                res.html +
                                '</div></div>'
                            ...
                            tab.data.push(route);
                            layui.ekb.render(tab.tabMenuTplId);
                    ...
                    layui.ekb.sidebarFocus(route.href);
                    return false
                },
                onChange: function () {
                }
            ...
            self.renderLayout = function (callback, url) {
                ...
                self.render(url, function (res) {
                    ...
                    if (conf.viewTabs === true) {
                        self.containerBody.addClass('ekb-tabs-body')
                    }
                    layui.ekb.appBody = self.containerBody;
